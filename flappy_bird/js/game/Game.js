...
export class Game {
    ...
    init() {
        ...
        this.updateScoreDisplay();
        console.log('Game initialized - Bird at:', birdX, birdY);
    }
    ...
    checkCollisions() {
        ...
        const bottom = this.bounds.bottom;
        if (this.bird.y < top || this.bird.y + this.bird.height > bottom) {
           this.onGameOver && this.onGameOver(); // или ваша логика окончания игры
           return;
       }

       // 2) Трубы
        for (const pipe of this.pipes) {
        if (typeof pipe.collidesWith === 'function' && pipe.collidesWith(this.bird)) {
            this.onGameOver && this.onGameOver(); // или ваша логика окончания игры
            return;
    }
  }
